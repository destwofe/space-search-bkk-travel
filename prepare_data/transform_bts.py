...
import pandas as pd
# data = pd.ExcelFile("DataBTS.xlsx", sheet_name='BTS(ราคา(บาท))')
...
dataCost = pd.read_excel("DataBTS.xlsx", sheet_name='BTS(ราคา(บาท))').to_dict()
dataTime = pd.read_excel("DataBTS.xlsx", sheet_name='BTS(เวลา(m))').to_dict()
...
with open('nodes-bts.json', 'w', encoding='utf8') as outfile:
